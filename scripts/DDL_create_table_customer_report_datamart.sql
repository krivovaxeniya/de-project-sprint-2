...
CREATE TABLE IF NOT EXISTS dwh.customer_report_datamart (
    ...
    customer_id BIGINT NOT NULL, -- идентификатор мастера
    customer_name VARCHAR NOT NULL, -- Ф. И. О. мастера
                        ...
                        *,
                        ...
                        FROM ( 
                            SELECT -- в этой выборке делаем расчёт по большинству столбцов, так как все они требуют одной и той же группировки, кроме столбца с самой популярной категорией товаров у мастера. Для этого столбца сделаем отдельную выборку с другой группировкой и выполним JOIN
                             ...
                             INNER JOIN (
                             		select distinct on(craftsman_id) craftsman_id, customer_id_for_craftsman_id, count_craftsman_id 
                             		from
                                    (SELECT     -- Эта выборка поможет определить самого популярного мастера у заказчика. Эта выборка не делается в предыдущем запросе, так как нужна другая группировка. Для данных этой выборки можно применить оконную функцию, которая и покажет самую популярную категорию товаров у мастера
                                            dd.customer_id AS customer_id_for_craftsman_id, 
                                            dd.craftsman_id, 
                                            COUNT(dd.craftsman_id) AS count_craftsman_id
                                            FROM dwh_delta AS dd
                                                GROUP BY dd.customer_id, dd.craftsman_id, dd.report_period
                                                    ORDER BY count_craftsman_id desc) AS a) AS T4 ON T2.customer_id = T4.customer_id_for_craftsman_id    
                ) AS T5 WHERE T5.rank_count_product = 1 and T5.rank_count_craftsman_id =1 ORDER BY report_period -- условие помогает оставить в выборке первую по популярности категорию товаров
)
,
                               ...
                               INNER JOIN (
                                    ...
                                    (SELECT     -- Эта выборка поможет определить самого популярного мастера у заказчика. Эта выборка не делается в предыдущем запросе, так как нужна другая группировка. Для данных этой выборки можно применить оконную функцию, которая и покажет самую популярную категорию товаров у мастера
                                            ...
                                            FROM dwh_delta AS dd
                                                GROUP BY dd.customer_id, dd.craftsman_id, dd.report_period
                                                    ORDER BY count_craftsman_id desc) AS a) AS T4 ON T2.customer_id = T4.customer_id_for_craftsman_id      
                ) AS T5 WHERE T5.rank_count_product = 1 and T5.rank_count_craftsman_id =1 ORDER BY report_period
),
